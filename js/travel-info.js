document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(
    ...
    .then((data) => {
      ...
      const travelInfo = traveldata.destinations.filter(
  ...
  fetch(
    ...
    .then((data) => {
      const travels = data;
      const travelRel = travels.destinations.filter(
        (travel) => travel.region === travelRegion && travel.name !== travelName
      );
        ...
        <div id="carouselExampleFade" class="carousel slide carousel-fade" data-bs-ride="carousel">
          <div class="carousel-inner travel-info__img-container">
            <div class="carousel-item active" data-bs-interval="4000">
              <img 
                src="${array.imageBanner[0]}" 
                class="d-block w-100 travel-info__img" 
                alt="${array.name}">
            </div>
            <div class="carousel-item" data-bs-interval="4000">
              <img 
                src="${array.imageBanner[1]}" 
                class="d-block w-100 travel-info__img" 
                alt="${array.name}">
            </div>
          </div>
      ...
      <div class="d-flex col-12 d-flex flex-lg-row flex-column-reverse container-xl" id="travel-info-container">
        <div class="col-12 col-lg-8 travel-info__include-container">
          ...
          <div id="info" class="travel-info__include-info d-none m-2">
            ...
            <p class="travel-info__include-items-container">
              ${includes(array.includes)}</p>
              
            <p class="travel-info__include-title"><strong>Itinerario:</strong></p>
            <p class="travel-info__include-items-container">
              ${includesItinerary(array.itinerary)}</p>
          </div>

          <div id="galeria" class="d-none travel-info__gallery-container my-2">
            ${includesGallery(array.gallery, array.name)}
          </div>
          ...
          <div id="opinion" class="d-none mx-3 mx-md-5 my-2 justify-content-center">
            ...
            <div class="my-md-4 mt-4 mt-md-5">
              <h5>Comparte tu experiencia aquí</h5>
              <div class="input-group my-2">
                <textarea class="form-control pb-5" placeholder="Describe tu experiencia en ${
                  array.name
                }"></textarea>
              </div>
              <div class="input-group my-2">
  ...
  function includesItinerary(array) {
    ...
    for (let i = 0; i < array.length; i++) {
      container +=
        `<p class="travel-info__include-items ps-2 pe-4">` + array[i] + `</p>`;
    }
  ...
  function includesGallery(array, altImg) {
    let container = "";

    for (let i = 0; i < array.length; i++) {
      container += `
        <div class="col-12 d-flex">
          <img src="${array[i]}" class="travel-info__gallery-img col-12" alt="${altImg}" />
        </div>`;
    }
